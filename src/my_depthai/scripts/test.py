...
'''
    ...
    @author Gerard Harkema + Tim (aangepast)
...
class detection_displayer:
    ...
    def __init__(self, config_file):
        rospy.loginfo("Laden config file: " + config_file)
        self.display_image = False  # Zet op True om live venster te tonen
        ...
        with open(config_file, 'r') as f:
            ...
            try:
                ...
                for class_name in self.class_names:
                    # Willekeurige kleuren maken als fallback
    ...
    def detections_callback(self, detections_data):
        if self.image_received and self.image is not None:
            # Loop over alle detecties en teken bounding boxes + info
            for detection in detections_data.detections:
                ...
                # Voeg tekst toe met naam, score en posities
                text = '%s: %.2f%%' % (self.class_names[class_index], detection.results[0].score * 100)
                cv2.putText(self.image, text, (int(x1)+5, int(y2)-5), cv2.FONT_HERSHEY_SIMPLEX, 1, class_color, 2, cv2.LINE_AA)
                cv2.putText(self.image, 'x: %.3f m' % x, (int(x1)+5, int(y2)+20), cv2.FONT_HERSHEY_SIMPLEX, 1, class_color, 2, cv2.LINE_AA)
                cv2.putText(self.image, 'y: %.3f m' % y, (int(x1)+5, int(y2)+45), cv2.FONT_HERSHEY_SIMPLEX, 1, class_color, 2, cv2.LINE_AA)
                cv2.putText(self.image, 'z: %.3f m' % z, (int(x1)+5, int(y2)+70), cv2.FONT_HERSHEY_SIMPLEX, 1, class_color, 2, cv2.LINE_AA)

            try:
                self.image_pub.publish(self.bridge.cv2_to_imgmsg(self.image, "bgr8"))
            except CvBridgeError as e:
                print("Fout bij publish image:", e)

            # Zet image_received terug naar False om op nieuwe beelden te wachten
            self.image_received = False
...
def main(args):
    rospy.init_node('publish_bounding_boxes', anonymous=True)

    node_name = rospy.get_name()

    nnConfig = rospy.get_param(node_name + '/nnConfig')  # pad naar config file
    resourceBaseFolder = rospy.get_param(node_name + '/resourceBaseFolder')

    config_path = resourceBaseFolder + '/' + nnConfig

    dd = detection_displayer(config_path)
